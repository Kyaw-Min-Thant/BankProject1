...
// from-WinHtut-to-lonelone-100
    ...
    int cur_time_counter = charCounting(cur_time);
    char from[5]={'F','r','o','m','-'};
    char to[4]={'-','t','o','-'};
    char aa[1]={'-'};
    if(who == 't'){
        ...
        for(int a=0; a<1; a++){
            db[transfer].trc[space_array[transfer]-15].note[index_point]=aa[a];
            index_point++;
        }
        ...
        for(int i=0; i<cur_time_counter; i++){
        ...
        for(int aaa=0; aaa<1; aaa++){
            db[receiver].trc[space_array[receiver]-15].note[index_point]=aa[aaa];
            index_point++;
        }
        for(int aaa=0; aaa<amount_counter; aaa++){
        ...
        for(int aaa=0; aaa<cur_time_counter; aaa++){
            db[receiver].trc[space_array[receiver]-15].note[index_point]=cur_time[aaa];
            index_point++;
        }
...
void addTime(){
    ...
    struct te *localTime = localtime(&currentTime);

    strftime(cur_time,10000,"%c",localTime);
